...
export async function selectFramework() {
    ...
    const groupedFrameworks: { [language: string]: string[] } = availableFrameworks.reduce((acc, { language, name }) => {
        if (!acc[language]) {
            acc[language] = [];
        }
    ...
    const frameworkOptions: vscode.QuickPickItem[] = Object.entries(groupedFrameworks).flatMap(([language, frameworks]) => {
        return [
            { label: language.toLowerCase(), kind: vscode.QuickPickItemKind.Separator },
            ...frameworks.map(framework => ({
                label: framework === currentFramework && language === currentLanguage ? `✓ ${framework}` : framework,
                description: language
            }))
        ];
    });

    const selected = await vscode.window.showQuickPick(frameworkOptions, { placeHolder: 'Select a framework' });
    ...
    if (selected) {
        ...
        if (frameworkName && language) {
            ...
            await config.update('framework.defaultLanguage', language, vscode.ConfigurationTarget.Workspace);
            await config.update('framework.defaultFramework', frameworkName, vscode.ConfigurationTarget.Workspace);
            vscode.window.showInformationMessage(`Selected framework: ${frameworkName} (${language})`);
        }
    }
}

// Function to load framework configuration
async function loadFrameworkConfig(): Promise<any> {
    const config = vscode.workspace.getConfiguration('shinyframeworks');
    const currentFramework = config.get<string>('framework.defaultFramework');
    const currentLanguage = config.get<string>('framework.defaultLanguage');
    ...
    try {
        const frameworkData = JSON.parse(await fs.readFile(frameworkFilePath, 'utf-8'));
        return { ...frameworkData, currentLanguage };
    } catch (error) {
        vscode.window.showErrorMessage(`Error loading framework configuration: ${error instanceof Error ? error.message : error}`);
        return null;
    }
}
...
async function getInterpreter(language: string): Promise<string | null> {
    ...
    if (language.toLowerCase() === 'python') {
        ...
        if (pythonPath) {
            ...
            vscode.window.showErrorMessage("Python interpreter not configured or found. Please set it up and try again.");
        }
        ...
        if (rPath) {
            ...
            vscode.window.showErrorMessage("R interpreter not configured or found. Please set it up and try again.");
        }
    }
...
async function runCommandInTerminal(interpreterPath: string, command: string, cwd: string): Promise<void> {
...
export async function createApp(uri: vscode.Uri) {
    ...
    if (!frameworkData || !frameworkData.app) {
        vscode.window.showErrorMessage('App configuration not found.');
        return;
    }

    const appConfig = frameworkData.app;
    if (!appConfig.code) {
    ...
    const uriPath = (await fs.stat(uri.fsPath)).isDirectory() ? uri : vscode.Uri.file(path.dirname(uri.fsPath));
    ...
    if (!interpreterPath) {
        vscode.window.showErrorMessage(`Interpreter for language "${frameworkData.currentLanguage}" is not configured.`);
        return;
    }

    // Execute the command in the terminal
    await runCommandInTerminal(interpreterPath, result.command, result.targetPath);
}

//Function to create Module
export async function createModule(uri: vscode.Uri) {
    const frameworkData = await loadFrameworkConfig();
    if (!frameworkData || !frameworkData.module) {
        vscode.window.showErrorMessage('Module configuration not found.');
        return;
    }

    const moduleConfig = frameworkData.module;
    ...
    if (moduleConfig.code) {
        const result = await prepareCommandWithRelativePath(uriPath, moduleConfig.code);
        if (!result) {
            return;
        }
        ...
        const targetDirectory = (await fs.stat(uri.fsPath)).isDirectory() ? uri.fsPath : path.dirname(uri.fsPath);
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Module file created successfully at ${filePath}.`);
        } catch (error) {
            vscode.window.showErrorMessage(`Error creating module file: ${error instanceof Error ? error.message : error}`);
        }
    } else {
        vscode.window.showErrorMessage('No code or file specified in module configuration.');
    }
}

export async function runAddin(uri: vscode.Uri) {
    const frameworkData = await loadFrameworkConfig();
    if (!frameworkData || !frameworkData.addins) {
        vscode.window.showErrorMessage('Add-ins configuration not found.');
        return;
    }

    const addins = frameworkData.addins;
    ...
    const selectedAddin = addins.find((addin: { name: string }) => addin.name === selectedAddinName);
    if (!selectedAddin) {
    ...
    if (selectedAddin.directory === 'file') {
        ...
        uriToPass = vscode.workspace.workspaceFolders?.[0]?.uri!;
        command = selectedAddin.code;
    } else {
        vscode.window.showErrorMessage('Invalid directory setting in add-in configuration.');
        return;
    }
    ...
    if (!command || !uriToPass) {
        vscode.window.showErrorMessage('Target directory or command could not be determined.');
        return;
    }
